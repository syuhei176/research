...
def dispute(
  ...
  assert PredicateInterface(self.ownershipPredicate).canInitiateExit(
    txHash,
    _txBytes,
    msg.sender,
    _segment)
...
def challenge(
  ...
  assert CommitmentChain(self.commitmentChain).verifyInclusion(
    _segment,
    _txBytes,
    _blkNum,
    _proof)
...
def secondDispute(
  ...
  assert PredicateInterface(self.ownershipPredicate).verifyDeprecation(
    txHash,
    _stateBytes,
    _txBytes,
    _sigs)
...
def finalizeDispute(
  ...
  (tokenAddress, decimalOffset) = PlasmaContract(self.plasmaContract).getTokenFromId(dispute.tokenId)
